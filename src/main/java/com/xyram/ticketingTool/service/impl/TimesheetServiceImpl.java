...
public class TimesheetServiceImpl implements TimesheetService {
	...
	public ApiResponse createTimeSheets(List<TimeSheet> timesheets) {
		...
		response = validateTimeSheet(timesheets);
			...
			if (timesheets.size() > 0) {
				for (TimeSheet sheet : timesheets) {
					...
					if (sheet.getProjectId() != null) {
						...
						projectMember = projectMemberRepository.getMemberInProject(currentUser.getScopeId(),
								sheet.getProjectId(), null);
	...
	public ApiResponse getAllMyTimeSheets(Map<String, Object> filter, Pageable pageable) {
		...
		String projectId = filter.containsKey("projectId") ? ((String) filter.get("projectId")) : null;
		String statusStr = filter.containsKey("status") ? ((String) filter.get("status")) : null;
		...
		Page<Map> timeSheetList = timesheetRepository.getAllMyTimeSheets(currentUser.getUserId(), projectId,
				fromDateStr, toDateStr, status, pageable);
...
//		content.put("totalCount", timesheetRepository.getAllMyTimeSheetsCount(currentUser.getUserId(), projectId, fromDateStr, toDateStr, statusStr));
	...
	public ApiResponse getAllMyTeamTimeSheets(Map<String, Object> filter, Pageable pageable) {
		...
		Page<Map> timeSheetList = timesheetRepository.getAllMyTeamTimeSheets(currentUser.getUserId(), employeeId,
...
//			content.put("totalCount", timesheetRepository.getAllMyTeamTimeSheetsCount(currentUser.getUserId(),employeeId, projectId, fromDateStr, toDateStr, statusStr));
	...
	public ReportExportResponse downloadAllMyTimeSheets(Map<String, Object> filter) {
		...
		List<Map> timeSheetList = timesheetRepository.downloadAllMyTimeSheets(currentUser.getUserId(), projectId,
	...
	public ReportExportResponse downloadAllMyTeamTimeSheets(Map<String, Object> filter) {
		...
		List<Map> timeSheetList = timesheetRepository.downloadAllMyTeamTimeSheets(currentUser.getUserId(), employeeId,
				projectId, fromDateStr, toDateStr, status);
		Map<String, Object> fileResponse = new HashMap<>();
	...
	private Workbook prepareExcelWorkBook(List<Map> storyDetails) {
		...
		/*
		 * t.timesheet_id as timeSheetId,t.employee_id as employeeId,t.timesheet_date as
		 * timeSheetDate, " +
		 ...
		 * + ",t.task_comments as comments, " + "t.reason as reason
		 */
		List<String> headers = Arrays.asList("ID", "Date", "Employee", "Project", "Task", "Task ID", "Description",
				"Hours Spent", "Comments", "Approver", "Status");
		...
		Workbook workbook = ExcelUtil
				.createSingleSheetWorkbook(ExcelUtil.createSheet("Time Sheet Report", headers, data));
	...
	public ApiResponse deleteTimeSheet(String timeSheetId) {
		...
		TimeSheet timeSheetObj = timesheetRepository.getByTimesheetId(timeSheetId);
		if (timeSheetObj != null) {
			...
			response.setSuccess(false);
			response.setMessage(ResponseMessages.TIME_SHEET_NOT_FOUND);
		}
		return response;
	}

	public ApiResponse validateTimeSheet(List<TimeSheet> timesheets) {
		ApiResponse response = new ApiResponse(false);
		for (TimeSheet sheet : timesheets) {
			if (sheet.getTaskName() == null || sheet.getTaskName().equals("")) {
				response.setSuccess(false);
				response.setMessage("Task name is mandatory");
				return response;
			} else if (sheet.getTaskName().length() < 3 || sheet.getTaskName().length() > 15) {
				response.setSuccess(false);
				response.setMessage("Task name length should be minimum 3 and maximum 15");
				return response;
			}

			if (sheet.getTaskDescription() == null || sheet.getTaskDescription().equals("")) {
				response.setSuccess(false);
				response.setMessage("Task description is mandatory");
				return response;
			} else if (sheet.getTaskDescription().length() < 3 || sheet.getTaskDescription().length() > 1000) {
				response.setSuccess(false);
				response.setMessage("Task description length should be minimum 3 and maximum 1000");
				return response;
			}
