...
//			+ "where a.ticket_description like %:searchString% ", nativeQuery = true)
	...
	@Query(value = "SELECT new map(jo.jobCode as jobCodes,jo.Id as  id,jo.jobTitle as jobTitle ) from JobOpenings j WHERE lower(j.jobTitle) LIKE %:searchString% ")
	List<Map> searchJobOpenings(String searchString);
