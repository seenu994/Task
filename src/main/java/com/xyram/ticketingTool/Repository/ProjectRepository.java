...
public interface ProjectRepository extends JpaRepository<Projects, String> {
	...
	@Query("Select new map(p.pId as id, p.projectName as PName, p.projectDescritpion as projectDescritpion, p.clientId as clientId,  "
			+ "p.inHouse as inHouse, p.status as status) from Projects p ")
