...
public interface StoryRepository extends JpaRepository<Story, String> {
	...
	@Query("select   new Map( s.storyNo as storyNo ,s.version as version,s.title as title , s.createdOn as createdOn, p.projectName as projectName,f.featureName as storyStatus,  "
			...
			+ " OR  s.storyType Like %:searchString% "
			+ " OR  CONCAT(e2.firstName ,' ', e2.lastName) LIKE %:searchString% "
			+ " OR s.title LIKE %:searchString% "
			+ " OR f.featureName LIKE %:searchString% "
			+ " OR pt.platformName LIKE %:searchString% "
...
//			+ "pt.platformName as platform, s.storyLabel as s.storyLabelId) 
	...
	@Query("select   new Map( s.storyNo as storyNo ,s.version as version,s.title as title "
			...
			+ "pt.platformName as platform, s.platform as platformId ,s.storyLabel as storyLabelId) "
			+ " From Story s" + " left join Projects p on p.pId =s.projectId "
			+ "left join Feature f on f.featureId=s.storyStatus " + "left join Employee e1 on e1.eId=s.owner "
			+ "left join Employee e2 on e2.eId=s.assignTo " + "left join StoryLabel sl on sl.id=s.storyLabel "
			+ " left join Platform pt on pt.id=s.platform " + " left join Version v on v.id= s.version "
			+ "left join Sprint sp on sp.id=s.sprintId " + " where s.id=:storyId and s.projectId =:projectId ")
...
//  Map  getAllStoriesByStoryId(String projectId ,String storyId);
	...
	@Query(value ="select  s.story_no as storyNo ,s.version as version,s.title as title , s.created_on as createdOn, p.project_name as projectName,f.feature_name as storyStatus,  "
					...
					+ "CONCAT(e2.frist_name ,' ', e2.last_name) as assignedTo   "
			+ "from story s "
			...
			+ "(:storyLabel is null or find_in_set(s.story_label,:storyLabel)) and "
			+ "(:searchString  is null "
			...
			+ " OR s.title LIKE %:searchString% "
			+ " OR pt.platform_name LIKE %:searchString% "
			+ " OR feature_name LIKE %:searchString% "
			+"  OR s.story_description Like %:searchString% )" ,nativeQuery=true)
	public List<Map> getStoryTesting(String projectId, String searchString, String assignTo ,
			String platform ,String storyStatus,String storyType,String storyLabel );
	
	
	@Query("select   new Map( s.storyNo as storyNo ,s.version as version,s.title as title , p.projectName as projectName,f.featureName as storyStatus,  "
