...
public interface TicketRepository extends JpaRepository<Ticket, String> {
	...
	@Query(value = "SELECT a.ticket_id as ticket_id, a.ticket_description as ticket_description, a.ticket_status as ticket_status, a.created_at as created_at, a.last_updated_at as last_updated_at, "
			...
			+ "where (:searchString is null or lower(a.ticket_description) LIKE %:searchString%)  OR (:searchString is null or lower(a.ticket_id) LIKE %:searchString%) OR (:priority is null or lower(p.priority_id) LIKE %:priority%)", nativeQuery = true)
	...
	*/	 
	...
	@Query("SELECT distinct new map(a.Id as ticket_id, a.ticketDescription as ticket_description, a.status as ticket_status, a.createdAt as created_at, a.createdBy as created_by, a.lastUpdatedAt as last_updated_at, "
			...
			+ "OR ('TICKETINGTOOL_ADMIN' = :roleId and a.status = :status) OR ('HR_ADMIN' = :roleId and a.status = :status and a.createdBy = :createdBy) OR ('INFRA_ADMIN' = :roleId and a.status = :status)) ORDER BY a.createdAt DESC")
	...
	@Query("SELECT distinct new map(a.Id as ticket_id, a.ticketDescription as ticket_description, a.status as ticket_status, a.createdAt as created_at, a.createdBy as created_by, a.lastUpdatedAt as last_updated_at, "
			...
			+ "OR ('TICKETINGTOOL_ADMIN' = :roleId and a.status IN ('COMPLETED', 'CANCELLED')) OR ('INFRA_ADMIN' = :roleId and a.status IN ('COMPLETED', 'CANCELLED')) OR ('HR_ADMIN' = :roleId and a.status IN ('COMPLETED', 'CANCELLED'))) ORDER BY a.createdAt DESC")
